...
class Math2D {
  calculateMiddlePoint = (firstPoint: CoordinateType, secondPoint: CoordinateType): CoordinateType => {
  ...
  calculateLinearPointFromTwoPoints = (firstPoint: CoordinateType, secondPoint: CoordinateType): LinearEquation => {
  ...
  calculateParallelEquation = (linearEquation: LinearEquation): LinearEquation => {
  ...
  calculatePerpendicularEquation = (linearEquation: LinearEquation): LinearEquation => {
    // Random a constance term from -50 -> 50
    const constantTerm = Math.floor(Math.random() * 100) - 50;
  ...
  calculateDistanceTwoPoints = (firstPoint: CoordinateType, secondPoint: CoordinateType): number => {
  ...
  calculateDistanceFromPointToLine = (point: CoordinateType, line: LinearEquation): number => {
  ...
  calculateParallelLineByPointAndLine = (point: CoordinateType, line: LinearEquation): LinearEquation => {
  ...
  calculatePerpendicularLineByPointAndLine = (point: CoordinateType, line: LinearEquation): LinearEquation => {
  ...
  calculateIntersectionByLineAndLine = (lineOne: LinearEquation, lineTwo: LinearEquation): CoordinateType => {
  ...
  calculateCircleEquationByCenterPoint = (centerPoint: CoordinateType, radius: number): CircleEquation => {
  ...
  calculateInternalBisectLineEquation = (lineOne: LinearEquation, lineTwo: LinearEquation): LinearEquation => {
    const firstLine = this._calculateBisectLineEquation(lineOne, lineTwo)[0];
    const secondLine = this._calculateBisectLineEquation(lineOne, lineTwo)[1];

    let pointInFirstLine;
    let pointInSecondLine;
    return this._getInternalBisectLineEquation();
  };

  _calculateBisectLineEquation = (lineOne: LinearEquation, lineTwo: LinearEquation): [LinearEquation, LinearEquation] => {
  ...
  */
  _getInternalBisectLineEquation = (lineOne: LinearEquation,
  ...
  _getPointInLine = (line: LinearEquation): CoordinateType => {
  ...
  _getPointInLineWithCondition = (line: LinearEquation, axis: string, comparison: string, point: CoordinateType): CoordinateType => {
    if (axis !== 'vertical'
       || axis !== 'horizontal'
       || comparison !== 'bigger'
       || comparison !== 'smaller') {
